...
export class ImageViewerComponent implements OnChanges, OnInit, AfterViewInit {
  ...
  isPDF() {
    ...
    return typeof this.getImagemAtual() === 'string' && (this.getImagemAtual().startsWith('JVBE') || this.getImagemAtual().startsWith('0M8R'));
  }

  isURlImagem() {
    return typeof this.getImagemAtual() === 'string' && this.getImagemAtual().match(new RegExp(/^(https|http|www\.)/g));
  }
