...
export class AdminKanbanComponent implements OnInit {
    ...
    refresh() {
      ...
      if (this.dashboardState) {
        this.goToBoard((this.project._id) ? this.project : this.user, (this.project._id) ? 1 : 0, this.callType);
      } else {
        this.getAllAdmin();
      }
      this.getProjectList();
    ...
    getDashboardByType(type) {
      this.boardType = type;
      this.goToBoard((this.project._id) ? this.project : this.user, type, this.callType);
    }
    ...
    goToBoard(lead, type, callType) {
      if(!lead._id){
        return this.common.showError('Select User Or Project');
      }
      ...
      this.taskStatusBarData[0].data = [];
      let params = '';
      (!type) ? params = `projectId=null&userId=${lead._id}&type=${type}&filter=null` : params = `projectId=${lead._id}&userId=null&type=${type}&filter=null`;
      this.common.loading++;
