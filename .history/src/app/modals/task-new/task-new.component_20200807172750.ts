...
export class TaskNewComponent implements OnInit {
  ...
  saveTask() {
    ...
    let CCUsers = [];
    this.normalTask.ccUsers.map(group=> {
      if(group.groupId){
        CCUsers = group.groupuser.filter(user => user._group_id === group.groupId).map((key)=> {return {user_id: key._id}});
      }else{
        CCUsers = this.normalTask.ccUsers.map(users => {return{user_id: users._id}})
      }});
